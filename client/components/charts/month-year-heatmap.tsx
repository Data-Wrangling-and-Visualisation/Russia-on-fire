...
interface MonthYearHeatmapProps {
  yearFilter?: number
}

export default function MonthYearHeatmap({ yearFilter }: MonthYearHeatmapProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null)
      try {
        const response = await fetch(`/api/fires?endpoint=stats/month-year`)
        if (!response.ok) {
        ...
        setError(error instanceof Error ? error.message : "Failed to load month-year data")
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    ...
    const y = d3
      .scaleBand()
      .domain(Array.from({ length: 12 }, (_, i) => (i + 1).toString()))
      .range([0, height])
      .padding(0.05)
    ...
    // Create color scale - using a more vibrant color scheme
    const color = d3.scaleSequential().interpolator(d3.interpolateViridis).domain([0, maxValue])
    ...
    svg
      .append("g")
      .call(d3.axisLeft(y).tickFormat((d) => monthNames[Number.parseInt(d.toString()) - 1]))
      .selectAll("text")
      .style("font-size", "10px")
    ...
    svg
      ...
      .text(`Monthly Fire ${metric === "count" ? "Incidents" : "Area"} by Year`)
    ...
    svg
      ...
      .attr("y", (d) => y(d.month.toString()) as number)
    ...
    svg
      ...
      .style("fill", "url(#legend-gradient-month)")
  ...
  if (loading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Monthly Fire Patterns</CardTitle>
      ...
      <CardHeader>
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
          <div>
            <CardTitle>Monthly Fire Patterns</CardTitle>
            <CardDescription>
              Heatmap showing fire {metric === "count" ? "incidents" : "area"} by month and year
